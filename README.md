...
При использовании опции `-b` (открыть в браузере) без указания `-o` создаются временные HTML-файлы. Для их удаления используйте команду `clean`:

```bash
# Показать список временных файлов без удаления
geocmp clean --dry-run

# Удалить все временные файлы, созданные geocmp
geocmp clean

# Удалить с подробным логированием
geocmp clean -v
```
...
- **Крестик (×)** - удалить слой из списка
...
- **Drag & Drop GeoJSON файлов** - перетащите .geojson файлы на карту для их добавления в список слоёв
